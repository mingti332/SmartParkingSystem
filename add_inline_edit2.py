...
# Find the end of reload Runnable and insert before "Button query = new Button"
pattern = r'(}\s*;\s*\n)(\s*Button query = new Button)'
match = re.search(pattern, content)
if match:
    print("Found pattern at position:", match.start())
    new_content = content[:match.start()+len(match.group(1))] + inline_edit_code + '\n' + content[match.start()+len(match.group(1)):]
    ...
    idx = content.find('Button query = new Button')
    print(f"'Button query = new Button' found at: {idx}")
    if idx > 0:
